          ...
          {currentView === 'welcome' && (
            <div className="text-center space-y-6 sm:space-y-8">
              <div className="flex items-center justify-center gap-3 mb-8">
                <Lightbulb className={`h-8 w-8 ${styles.accent}`} />
                <h1 className={`text-4xl font-pixel drop-shadow-lg ${styles.text}`}>
              ...
              <div className="space-y-4 sm:space-y-6 max-w-3xl mx-auto">
                {welcomeText.split('\n\n').map((paragraph, index) => (
                  <p key={index} className={`${styles.text} leading-relaxed text-base sm:text-lg font-pixel`}>
                    {paragraph}
                  </p>
                ))}
              </div>
              <button 
                ...
                className={`px-8 py-3 rounded-lg font-pixel border-2 border-black/30 transition-all duration-200 transform hover:scale-105 ${styles.button}`}
              >
          ...
          {currentView === 'list' && (
            <div className="space-y-6">
              <div className="text-center space-y-2">
                <h3 className={`text-2xl sm:text-3xl font-pixel font-bold ${styles.text}`}>
                  {language === 'deutsch' ? 'Meine Ideen' : 'My Ideas'}
                </h3>
                <p className={`${styles.text} font-pixel`}>
                  {language === 'deutsch' 
              ...
              <div className="grid gap-4 sm:gap-6">
                {ideas.map((idea) => {
                  ...
                  return (
                    <div key={idea.id} className={`p-6 border-2 rounded-lg transition-all duration-200 hover:scale-[1.02] ${styles.cardBg}`}>
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                        <div className="space-y-2 flex-1">
                          <h4 className={`text-lg sm:text-xl ${styles.text} font-pixel font-semibold`}>{idea.title}</h4>
                          <p className={`text-sm sm:text-base ${styles.text} font-pixel leading-relaxed`}>{idea.shortDescription}</p>
                        </div>
                        {avgRating !== null && (
                          <div className="text-left sm:text-right space-y-2 sm:ml-6">
                            {renderStars(avgRating)}
                            <p className={`text-xs sm:text-sm ${styles.text} font-pixel`}>
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 mt-4">
                        <button 
                          ...
                          className={`flex-1 px-4 py-2 text-sm sm:text-base font-pixel rounded-lg transition-all duration-200 border-2 ${styles.cardBg} ${styles.text} hover:bg-blue-400/10 border-black/30`}
                        >
                          {language === 'deutsch' ? 'Details ansehen' : 'View Details'}
                        </button>
                        <button 
                          onClick={() => openPrototype(idea)}
                          className={`px-4 py-2 text-sm sm:text-base font-pixel rounded-lg transition-all duration-200 transform hover:scale-105 border-2 border-black/30 flex items-center justify-center gap-2 ${styles.button}`}
                        >
